...
export const Posts: React.FC<PostProps> = React.memo(({ post, handleClick, GotouserInfo, savePost, fills,formatDate }) => {
    return (
      <div key={post.id} className="h-[19rem] mx-[30px] border-y-[1px] border-[#F2F2F2] pt-10 space-y-2">
        {/* Author */}
        ...
        <div className="flex pt-3 cursor-pointer" onClick={() => handleClick(post.id)}>
          ...
          <div className="flex flex-col w-[80%]">
            <div className="text-2xl font-extrabold w-[90%]">{post.title}</div>
        ...
        <div className="flex px-7 pt-5 justify-between">
          ...
          <div className="flex items-center space-x-10 cursor-pointer" onClick={() => handleClick(post.id)}>
            <div className="text-sm font-light">{formatDate(post.date)}</div>
          ...
          <div className="flex items-center space-x-9">
            <div>
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6 cursor-pointer z-[1000]"
